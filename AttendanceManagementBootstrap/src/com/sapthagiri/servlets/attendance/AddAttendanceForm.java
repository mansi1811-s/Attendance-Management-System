...
public class AddAttendanceForm extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<html>");
		out.println("<head>");
		out.println("<title>Add Attendance</title>");
		...
		out.println("$.get(\"facultylistsubjectajax\", function(responseJson) {");
		 ...
		 * out.println("var tempId = branch.branchId"); out.println(
		 * "$(\"<option>\").val(tempId).text(branch.branchName).appendTo($select);");
		...
		if(session==null||session.getAttribute("faculty")==null){
			...
			request.getRequestDispatcher("AddAttendanceForm.html").include(request, response);
		}
